...
interface ShipmentDetailsModalProps {
  ...
  onClose: () => void;
  onSave?: (mode: "create" | "update", shipment: UIShipment) => void;
  onDelete?: (shipment: UIShipment) => void;
...
export default function ShipmentDetailsModal({
  ...
  const isNew = !shipment?.id || shipment.id === "new";
      ...
      <div style={modalStyle}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <div style={{ fontSize: 14, fontWeight: 750 }}>
                {isNew ? "Create Shipment" : "Shipment Detail"}
              </div>
              <div style={{ fontSize: 12, opacity: 0.9 }}>
                {isNew ? "Add a new shipment record" : "Review and update shipment fields"}
              </div>
            </div>
          </div>
          ...
          <section style={{ marginBottom: 18 }}>
            ...
            <div style={gridStyle}>
              ...
              <div>
                ...
                <div style={inputWrapStyle}>
                  <span style={iconStyle}>
                    {containerLocked ? <Lock size={16} /> : <Box size={16} />}
                  </span>
                  <input
                    ...
                    style={{ ...inputStyle, background: containerLocked ? "#f8fafc" : "#fff" }}
                    disabled={isSaving || containerLocked}
                    placeholder="e.g. MSCU1234567"
                  />
                </div>
                {!isNew ? (
                  <div style={{ marginTop: 6, fontSize: 11, color: "#64748b" }}>
                    Container # is locked after creation.
                  </div>
                ) : null}
              </div>
              ...
              {/* The rest unchanged (container size, dates, status, etc.) */}
              {/* Container Size */}
              <div>
        ...
        <div style={footerStyle}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <button
              type="button"
              onClick={() => onSave?.(isNew ? "create" : "update", form)}
              disabled={isSaving || !onSave || !canSave}
              style={{
                ...btnStyle,
                background: isSaving || !onSave || !canSave ? "#93c5fd" : "#2563eb",
                color: "#fff",
              }}
            >
              {isSaving ? "Saving…" : isNew ? "Create Shipment" : "Update Shipment"}
            </button>
          </div>
        </div>
      </div>
