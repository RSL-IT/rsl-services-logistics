...
-- 1) Ensure shipment notes table is renamed and uses containerId.
DO $$
BEGIN
  IF to_regclass('public."tbl_shipmentNotes"') IS NOT NULL
     AND to_regclass('public."tbl_containerNotes"') IS NULL THEN
...
CREATE INDEX IF NOT EXISTS "tbl_containerNotes_containerId_idx"
  ON "tbl_containerNotes"("containerId");

-- 2) Ensure shipment->purchaseOrder join table is renamed and normalized to containerID FK.
DO $$
BEGIN
...
BEGIN
  IF to_regclass('public."tbljn_shipment_purchaseOrder"') IS NOT NULL
     AND to_regclass('public."tbljn_container_purchaseOrder"') IS NOT NULL THEN
    ...
    IF EXISTS (
      ...
      FROM information_schema.columns
      WHERE table_schema = 'public'
        AND table_name = 'tbljn_shipment_purchaseOrder'
        AND column_name = 'shipmentID'
    ) THEN
      INSERT INTO "tbljn_container_purchaseOrder" ("containerID", "purchaseOrderGID")
      SELECT c.id, old."purchaseOrderGID"
      FROM "tbljn_shipment_purchaseOrder" old
      JOIN "tbl_container" c
        ON c."containerNumber" = old."shipmentID"
      WHERE old."purchaseOrderGID" IS NOT NULL
        AND NOT EXISTS (
          SELECT 1
          FROM "tbljn_container_purchaseOrder" cur
          WHERE cur."containerID" = c.id
            AND cur."purchaseOrderGID" = old."purchaseOrderGID"
        );
    END IF;

    EXECUTE 'DROP TABLE "tbljn_shipment_purchaseOrder"';
  END IF;
END
$$;
  ...
  IF to_regclass('public."tbljn_container_purchaseOrder"') IS NOT NULL THEN
    ...
    IF EXISTS (
      ...
      WHERE c."containerNumber" = j."shipmentID"
        AND j."containerID" IS NULL;
    END IF;

    DELETE FROM "tbljn_container_purchaseOrder" WHERE "containerID" IS NULL OR "purchaseOrderGID" IS NULL;

    DELETE FROM "tbljn_container_purchaseOrder" j
    WHERE NOT EXISTS (
      SELECT 1 FROM "tbl_container" c WHERE c.id = j."containerID"
    );

    DELETE FROM "tbljn_container_purchaseOrder" j
    WHERE NOT EXISTS (
      SELECT 1 FROM "tbl_purchaseOrder" po WHERE po."purchaseOrderGID" = j."purchaseOrderGID"
    );
...
CREATE UNIQUE INDEX IF NOT EXISTS "tbljn_container_purchaseOrder_containerID_purchaseOrderGID_key"
  ON "tbljn_container_purchaseOrder"("containerID", "purchaseOrderGID");
CREATE INDEX IF NOT EXISTS "tbljn_container_purchaseOrder_containerID_idx"
  ON "tbljn_container_purchaseOrder"("containerID");
CREATE INDEX IF NOT EXISTS "tbljn_container_purchaseOrder_purchaseOrderGID_idx"
  ON "tbljn_container_purchaseOrder"("purchaseOrderGID");

-- 3) Ensure container PO-product allocation table exists and has constraints.
...
DELETE FROM "tbljn_container_purchaseOrder_rslProduct" j
WHERE NOT EXISTS (
  SELECT 1 FROM "tlkp_rslProduct" p WHERE p."shortName" = j."rslProductID"
);
